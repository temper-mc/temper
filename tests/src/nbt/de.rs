...
use ionic_nbt::de::converter::FromNbt;

#[test]
#[ignore]
fn test_basic_get_functions() {
    // let data = include_bytes!("../../../../.etc/TheAIguy_.nbt");
    // let data = ionic_nbt::decompress_gzip(data).unwrap();

    // let mut parser = ionic_nbt::de::borrow::NbtTape::new(data.as_slice());
    // parser.parse();
...
fn create_derive_outline() {
    ...
    let mut tape = ionic_nbt::de::borrow::NbtTape::new(data.as_slice());
    tape.parse();

    mod structs {
        #![allow(dead_code)]

        use ionic_nbt::de::borrow::{NbtTape, NbtTapeElement};
        use ionic_nbt::de::converter::FromNbt;
        ...
        impl FromNbt<'_> for BigTest {
            fn from_nbt(tapes: &NbtTape, element: &NbtTapeElement) -> ionic_nbt::Result<Self> {
