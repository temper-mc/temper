...
pub fn handle_start_digging(
    ...
    for event in events.read() {
            ...
            .get_or_generate_chunk(pos.chunk(), Dimension::Overworld)
        {
            ...
            Err(e) => {
                warn!(
                    "StartDigging: Failed to load/generate chunk for {:?}: {:?}",
                    event.position, e
                );
                continue;
            }
    ...
    for event in events.read() {
        ...
        if elapsed < digging.break_time {
                ...
                .get_or_generate_chunk(pos.chunk(), Dimension::Overworld)
            {
                ...
                Err(e) => {
                    error!(
                        "Failed to load/generate chunk for anti-cheat revert: {:?}",
                        e
                    );
                    continue;
                }
            };
            let real_block_state = chunk.get_block(pos.chunk_block_pos());
