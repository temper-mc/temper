...
impl SnifferBundle {
    pub fn new(position: Position) -> Self {
        let metadata = EntityMetadata::from_vanilla(&VanillaEntityType::SNIFFER);
...
mod tests {
    ...
    #[test]
    fn test_sniffer_bundle_creation() {
        let position = Position::new(0.0, 64.0, 0.0);
        let sniffer = SnifferBundle::new(position);

        assert_eq!(sniffer.metadata.protocol_id(), 114);
        assert_eq!(sniffer.metadata.resource_name(), "sniffer");
        assert!(sniffer.metadata.is_mob());
        assert!(sniffer.spawn.is_persistent());
    }

    #[test]
    fn test_sniffer_bundle_with_rotation() {
        let position = Position::new(10.0, 70.0, 20.0);
        let rotation = Rotation {
            yaw: 90.0,
            pitch: 0.0,
        };
        let sniffer = SnifferBundle::with_rotation(position, rotation);
        assert_eq!(sniffer.rotation.yaw, 90.0);
    }
}
