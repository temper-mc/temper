...
impl MooshroomBundle {
    pub fn new(position: Position) -> Self {
        let metadata = EntityMetadata::from_vanilla(&VanillaEntityType::MOOSHROOM);
...
mod tests {
    ...
    #[test]
    fn test_mooshroom_bundle_creation() {
        let position = Position::new(0.0, 64.0, 0.0);
        let mooshroom = MooshroomBundle::new(position);

        assert_eq!(mooshroom.metadata.protocol_id(), 83);
        assert_eq!(mooshroom.metadata.resource_name(), "mooshroom");
        assert!(mooshroom.metadata.is_mob());
        assert!(mooshroom.spawn.is_persistent());
    }

    #[test]
    fn test_mooshroom_bundle_with_rotation() {
        let position = Position::new(10.0, 70.0, 20.0);
        let rotation = Rotation {
            yaw: 90.0,
            pitch: 0.0,
        };
        let mooshroom = MooshroomBundle::with_rotation(position, rotation);
        assert_eq!(mooshroom.rotation.yaw, 90.0);
    }
}
