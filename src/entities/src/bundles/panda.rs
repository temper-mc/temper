...
impl PandaBundle {
    pub fn new(position: Position) -> Self {
        let metadata = EntityMetadata::from_vanilla(&VanillaEntityType::PANDA);
...
mod tests {
    ...
    #[test]
    fn test_panda_bundle_creation() {
        let position = Position::new(0.0, 64.0, 0.0);
        let panda = PandaBundle::new(position);

        assert_eq!(panda.metadata.protocol_id(), 92);
        assert_eq!(panda.metadata.resource_name(), "panda");
        assert!(panda.metadata.is_mob());
        assert!(panda.spawn.is_persistent());
    }

    #[test]
    fn test_panda_bundle_with_rotation() {
        let position = Position::new(10.0, 70.0, 20.0);
        let rotation = Rotation {
            yaw: 90.0,
            pitch: 0.0,
        };
        let panda = PandaBundle::with_rotation(position, rotation);
        assert_eq!(panda.rotation.yaw, 90.0);
    }
}
