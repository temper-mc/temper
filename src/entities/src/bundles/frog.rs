...
impl FrogBundle {
    pub fn new(position: Position) -> Self {
        let metadata = EntityMetadata::from_vanilla(&VanillaEntityType::FROG);
...
mod tests {
    ...
    #[test]
    fn test_frog_bundle_creation() {
        let position = Position::new(0.0, 64.0, 0.0);
        let frog = FrogBundle::new(position);

        assert_eq!(frog.metadata.protocol_id(), 53);
        assert_eq!(frog.metadata.resource_name(), "frog");
        assert!(frog.metadata.is_mob());
        assert!(frog.spawn.is_persistent());
    }
