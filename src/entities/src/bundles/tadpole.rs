...
impl TadpoleBundle {
    pub fn new(position: Position) -> Self {
        let metadata = EntityMetadata::from_vanilla(&VanillaEntityType::TADPOLE);
...
mod tests {
    ...
    #[test]
    fn test_tadpole_bundle_creation() {
        let position = Position::new(0.0, 64.0, 0.0);
        let tadpole = TadpoleBundle::new(position);

        assert_eq!(tadpole.metadata.protocol_id(), 125);
        assert_eq!(tadpole.metadata.resource_name(), "tadpole");
        assert!(tadpole.metadata.is_mob());
        assert!(tadpole.spawn.is_persistent());
    }

    #[test]
    fn test_tadpole_bundle_with_rotation() {
        let position = Position::new(10.0, 70.0, 20.0);
        let rotation = Rotation {
            yaw: 90.0,
            pitch: 0.0,
        };
        let tadpole = TadpoleBundle::with_rotation(position, rotation);
        assert_eq!(tadpole.rotation.yaw, 90.0);
    }
}
