...
impl OcelotBundle {
    pub fn new(position: Position) -> Self {
        let metadata = EntityMetadata::from_vanilla(&VanillaEntityType::OCELOT);
...
mod tests {
    ...
    #[test]
    fn test_ocelot_bundle_creation() {
        let position = Position::new(0.0, 64.0, 0.0);
        let ocelot = OcelotBundle::new(position);

        assert_eq!(ocelot.metadata.protocol_id(), 87);
        assert_eq!(ocelot.metadata.resource_name(), "ocelot");
        assert!(ocelot.metadata.is_mob());
        assert!(ocelot.spawn.is_persistent());
    }

    #[test]
    fn test_ocelot_bundle_with_rotation() {
        let position = Position::new(10.0, 70.0, 20.0);
        let rotation = Rotation {
            yaw: 90.0,
            pitch: 0.0,
        };
        let ocelot = OcelotBundle::with_rotation(position, rotation);
        assert_eq!(ocelot.rotation.yaw, 90.0);
    }
}
