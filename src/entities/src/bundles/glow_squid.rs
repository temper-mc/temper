...
impl GlowSquidBundle {
    pub fn new(position: Position) -> Self {
        let metadata = EntityMetadata::from_vanilla(&VanillaEntityType::GLOW_SQUID);
...
mod tests {
    ...
    #[test]
    fn test_glow_squid_bundle_creation() {
        let position = Position::new(0.0, 64.0, 0.0);
        let glow_squid = GlowSquidBundle::new(position);

        assert_eq!(glow_squid.metadata.protocol_id(), 59);
        assert_eq!(glow_squid.metadata.resource_name(), "glow_squid");
        assert!(glow_squid.metadata.is_mob());
    }

    #[test]
    fn test_glow_squid_bundle_with_rotation() {
        let position = Position::new(10.0, 70.0, 20.0);
        let rotation = Rotation {
            yaw: 90.0,
            pitch: 0.0,
        };
        let glow_squid = GlowSquidBundle::with_rotation(position, rotation);
        assert_eq!(glow_squid.rotation.yaw, 90.0);
    }
}
