...
impl StriderBundle {
    pub fn new(position: Position) -> Self {
        let metadata = EntityMetadata::from_vanilla(&VanillaEntityType::STRIDER);
...
mod tests {
    ...
    #[test]
    fn test_strider_bundle_creation() {
        let position = Position::new(0.0, 64.0, 0.0);
        let strider = StriderBundle::new(position);

        assert_eq!(strider.metadata.protocol_id(), 124);
        assert_eq!(strider.metadata.resource_name(), "strider");
        assert!(strider.metadata.is_mob());
        assert!(strider.physical.fire_immune);
        assert!(strider.spawn.is_persistent());
    }

    #[test]
    fn test_strider_bundle_with_rotation() {
        let position = Position::new(10.0, 70.0, 20.0);
        let rotation = Rotation {
            yaw: 90.0,
            pitch: 0.0,
        };
        let strider = StriderBundle::with_rotation(position, rotation);
        assert_eq!(strider.rotation.yaw, 90.0);
    }
}
