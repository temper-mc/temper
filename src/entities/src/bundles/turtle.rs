...
impl TurtleBundle {
    pub fn new(position: Position) -> Self {
        let metadata = EntityMetadata::from_vanilla(&VanillaEntityType::TURTLE);
...
mod tests {
    ...
    #[test]
    fn test_turtle_bundle_creation() {
        let position = Position::new(0.0, 64.0, 0.0);
        let turtle = TurtleBundle::new(position);

        assert_eq!(turtle.metadata.protocol_id(), 132);
        assert_eq!(turtle.metadata.resource_name(), "turtle");
        assert!(turtle.metadata.is_mob());
        assert!(turtle.spawn.is_persistent());
    }
