...
impl PufferfishBundle {
    pub fn new(position: Position) -> Self {
        let metadata = EntityMetadata::from_vanilla(&VanillaEntityType::PUFFERFISH);
...
mod tests {
    ...
    #[test]
    fn test_pufferfish_bundle_creation() {
        let position = Position::new(0.0, 64.0, 0.0);
        let pufferfish = PufferfishBundle::new(position);

        assert_eq!(pufferfish.metadata.protocol_id(), 102);
        assert_eq!(pufferfish.metadata.resource_name(), "pufferfish");
        assert!(pufferfish.metadata.is_mob());
        // WATER_AMBIENT is not persistent
        assert!(!pufferfish.spawn.is_persistent());
    }

    #[test]
    fn test_pufferfish_bundle_with_rotation() {
        let position = Position::new(10.0, 70.0, 20.0);
        let rotation = Rotation {
            yaw: 90.0,
            pitch: 0.0,
        };
        let pufferfish = PufferfishBundle::with_rotation(position, rotation);
        assert_eq!(pufferfish.rotation.yaw, 90.0);
    }
}
