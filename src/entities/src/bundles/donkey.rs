use bevy_ecs::prelude::Bundle;
use ionic_components::entity_identity::EntityIdentity;
use ionic_components::player::grounded::OnGround;
use ionic_components::player::position::Position;
use ionic_components::player::rotation::Rotation;
use ionic_components::player::velocity::Velocity;
use ionic_data::generated::entities::EntityType as VanillaEntityType;

use crate::components::{
    CombatProperties, EntityMetadata, LastSyncedPosition, PhysicalProperties, SpawnProperties,
};

/// Complete bundle to spawn a donkey in Bevy ECS.
///
/// This bundle contains all the necessary components to represent a donkey
/// in the world. It uses Vanilla's data from ionic-data to correctly
/// initialize properties.
#[derive(Bundle)]
pub struct DonkeyBundle {
    pub identity: EntityIdentity,
    pub metadata: EntityMetadata,
    pub physical: PhysicalProperties,
    pub combat: CombatProperties,
    pub spawn: SpawnProperties,
    pub position: Position,
    pub rotation: Rotation,
    pub velocity: Velocity,
    pub on_ground: OnGround,
    pub last_synced_position: LastSyncedPosition,
}

impl DonkeyBundle {
    pub fn new(position: Position) -> Self {
        let metadata = EntityMetadata::from_vanilla(&VanillaEntityType::DONKEY);
        let physical = PhysicalProperties::from_metadata(&metadata);
        let combat = CombatProperties::from_metadata(&metadata);
        let spawn = SpawnProperties::from_metadata(&metadata);

        Self {
            identity: EntityIdentity::new(),
            metadata,
            physical,
            combat,
            spawn,
            rotation: Rotation::default(),
            velocity: Velocity::zero(),
            on_ground: OnGround(false),
            last_synced_position: LastSyncedPosition::from_position(&position),
            position,
        }
    }

    pub fn with_rotation(position: Position, rotation: Rotation) -> Self {
        let mut bundle = Self::new(position);
        bundle.rotation = rotation;
        bundle
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn test_donkey_bundle_creation() {
        const EPSILON_F32: f32 = 1e-6;
        const EPSILON_F64: f64 = 1e-6;

        let position = Position::new(0.0, 64.0, 0.0);
        let donkey = DonkeyBundle::new(position);

        // Verify vanilla metadata
        assert_eq!(donkey.metadata.protocol_id(), 34);
        assert_eq!(donkey.metadata.resource_name(), "donkey");
        assert!(donkey.metadata.is_mob());

        // Verify physical properties (using epsilon for floating point comparison)
        // Donkey dimensions: [width=1.3964844, height=1.5]
        assert!((donkey.physical.bounding_box.height() - 1.5).abs() < EPSILON_F64);
        assert!((donkey.physical.bounding_box.width() - 1.3964844).abs() < EPSILON_F64);
        assert!((donkey.physical.eye_height - 1.425).abs() < EPSILON_F32);
        assert!(!donkey.physical.fire_immune);

        // Verify combat properties
        assert!(donkey.combat.attackable);
        assert_eq!(donkey.combat.invulnerability_ticks, 0);

        // Verify spawn properties (CREATURE category - persistent)
        assert!(donkey.spawn.saveable);
        assert_eq!(donkey.spawn.limit_per_chunk, 6);
        assert!(donkey.spawn.is_friendly());
        assert!(donkey.spawn.is_persistent());
    }

    #[test]
    fn test_donkey_bundle_with_rotation() {
        let position = Position::new(10.0, 70.0, 20.0);
        let rotation = Rotation {
            yaw: 90.0,
            pitch: 0.0,
        };
        let donkey = DonkeyBundle::with_rotation(position, rotation);

        assert_eq!(donkey.rotation.yaw, 90.0);
        assert_eq!(donkey.rotation.pitch, 0.0);
    }
}
