...
/// Complete bundle to spawn an axolotl in Bevy ECS.
///
/// This bundle contains all the necessary components to represent an axolotl
...
impl AxolotlBundle {
    pub fn new(position: Position) -> Self {
        let metadata = EntityMetadata::from_vanilla(&VanillaEntityType::AXOLOTL);
...
mod tests {
    ...
    #[test]
    fn test_axolotl_bundle_creation() {
        const EPSILON_F32: f32 = 1e-6;
        const EPSILON_F64: f64 = 1e-6;

        let position = Position::new(0.0, 64.0, 0.0);
        let axolotl = AxolotlBundle::new(position);

        // Verify vanilla metadata
        assert_eq!(axolotl.metadata.protocol_id(), 7);
        assert_eq!(axolotl.metadata.resource_name(), "axolotl");
        assert!(axolotl.metadata.is_mob());

        // Verify physical properties (using epsilon for floating point comparison)
        // Axolotl dimensions: [width=0.75, height=0.42]
        assert!((axolotl.physical.bounding_box.height() - 0.42).abs() < EPSILON_F64);
        assert!((axolotl.physical.bounding_box.width() - 0.75).abs() < EPSILON_F64);
        assert!((axolotl.physical.eye_height - 0.2751).abs() < EPSILON_F32);
        assert!(!axolotl.physical.fire_immune);

        // Verify combat properties
        assert!(axolotl.combat.attackable);
        assert_eq!(axolotl.combat.invulnerability_ticks, 0);

        // Verify spawn properties
        assert!(axolotl.spawn.saveable);
        assert_eq!(axolotl.spawn.limit_per_chunk, 4);
        assert!(axolotl.spawn.is_friendly());
        assert!(!axolotl.spawn.is_persistent());
    }

    #[test]
    fn test_axolotl_bundle_with_rotation() {
        let position = Position::new(10.0, 70.0, 20.0);
        let rotation = Rotation {
            yaw: 90.0,
            pitch: 0.0,
        };
        let axolotl = AxolotlBundle::with_rotation(position, rotation);

        assert_eq!(axolotl.rotation.yaw, 90.0);
        assert_eq!(axolotl.rotation.pitch, 0.0);
    }
}
