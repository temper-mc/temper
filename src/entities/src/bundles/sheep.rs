...
impl SheepBundle {
    pub fn new(position: Position) -> Self {
        let metadata = EntityMetadata::from_vanilla(&VanillaEntityType::SHEEP);
...
mod tests {
    ...
    #[test]
    fn test_sheep_bundle_creation() {
        let position = Position::new(0.0, 64.0, 0.0);
        let sheep = SheepBundle::new(position);

        assert_eq!(sheep.metadata.protocol_id(), 106);
        assert_eq!(sheep.metadata.resource_name(), "sheep");
        assert!(sheep.metadata.is_mob());
        assert!(sheep.spawn.is_persistent());
    }

    #[test]
    fn test_sheep_bundle_with_rotation() {
        let position = Position::new(10.0, 70.0, 20.0);
        let rotation = Rotation {
            yaw: 90.0,
            pitch: 0.0,
        };
        let sheep = SheepBundle::with_rotation(position, rotation);
        assert_eq!(sheep.rotation.yaw, 90.0);
    }
}
