...
impl SalmonBundle {
    pub fn new(position: Position) -> Self {
        let metadata = EntityMetadata::from_vanilla(&VanillaEntityType::SALMON);
...
mod tests {
    ...
    #[test]
    fn test_salmon_bundle_creation() {
        let position = Position::new(0.0, 64.0, 0.0);
        let salmon = SalmonBundle::new(position);

        assert_eq!(salmon.metadata.protocol_id(), 105);
        assert_eq!(salmon.metadata.resource_name(), "salmon");
        assert!(salmon.metadata.is_mob());
        // WATER_AMBIENT is not persistent
        assert!(!salmon.spawn.is_persistent());
    }

    #[test]
    fn test_salmon_bundle_with_rotation() {
        let position = Position::new(10.0, 70.0, 20.0);
        let rotation = Rotation {
            yaw: 90.0,
            pitch: 0.0,
        };
        let salmon = SalmonBundle::with_rotation(position, rotation);
        assert_eq!(salmon.rotation.yaw, 90.0);
    }
}
