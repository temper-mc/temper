...
impl MuleBundle {
    pub fn new(position: Position) -> Self {
        let metadata = EntityMetadata::from_vanilla(&VanillaEntityType::MULE);
...
mod tests {
    ...
    #[test]
    fn test_mule_bundle_creation() {
        let position = Position::new(0.0, 64.0, 0.0);
        let mule = MuleBundle::new(position);

        assert_eq!(mule.metadata.protocol_id(), 84);
        assert_eq!(mule.metadata.resource_name(), "mule");
        assert!(mule.metadata.is_mob());
        assert!(mule.spawn.is_persistent());
    }

    #[test]
    fn test_mule_bundle_with_rotation() {
        let position = Position::new(10.0, 70.0, 20.0);
        let rotation = Rotation {
            yaw: 90.0,
            pitch: 0.0,
        };
        let mule = MuleBundle::with_rotation(position, rotation);
        assert_eq!(mule.rotation.yaw, 90.0);
    }
}
