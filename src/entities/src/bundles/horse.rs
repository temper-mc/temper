...
impl HorseBundle {
    pub fn new(position: Position) -> Self {
        let metadata = EntityMetadata::from_vanilla(&VanillaEntityType::HORSE);
...
mod tests {
    ...
    #[test]
    fn test_horse_bundle_creation() {
        let position = Position::new(0.0, 64.0, 0.0);
        let horse = HorseBundle::new(position);

        assert_eq!(horse.metadata.protocol_id(), 64);
        assert_eq!(horse.metadata.resource_name(), "horse");
        assert!(horse.metadata.is_mob());
        assert!(horse.spawn.is_persistent());
    }

    #[test]
    fn test_horse_bundle_with_rotation() {
        let position = Position::new(10.0, 70.0, 20.0);
        let rotation = Rotation {
            yaw: 90.0,
            pitch: 0.0,
        };
        let horse = HorseBundle::with_rotation(position, rotation);
        assert_eq!(horse.rotation.yaw, 90.0);
    }
}
