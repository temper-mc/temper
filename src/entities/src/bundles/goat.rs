...
impl GoatBundle {
    pub fn new(position: Position) -> Self {
        let metadata = EntityMetadata::from_vanilla(&VanillaEntityType::GOAT);
...
mod tests {
    ...
    #[test]
    fn test_goat_bundle_creation() {
        let position = Position::new(0.0, 64.0, 0.0);
        let goat = GoatBundle::new(position);

        assert_eq!(goat.metadata.protocol_id(), 60);
        assert_eq!(goat.metadata.resource_name(), "goat");
        assert!(goat.metadata.is_mob());
        assert!(goat.spawn.is_persistent());
    }

    #[test]
    fn test_goat_bundle_with_rotation() {
        let position = Position::new(10.0, 70.0, 20.0);
        let rotation = Rotation {
            yaw: 90.0,
            pitch: 0.0,
        };
        let goat = GoatBundle::with_rotation(position, rotation);
        assert_eq!(goat.rotation.yaw, 90.0);
    }
}
