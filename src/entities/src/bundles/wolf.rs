...
impl WolfBundle {
    pub fn new(position: Position) -> Self {
        let metadata = EntityMetadata::from_vanilla(&VanillaEntityType::WOLF);
...
mod tests {
    ...
    #[test]
    fn test_wolf_bundle_creation() {
        let position = Position::new(0.0, 64.0, 0.0);
        let wolf = WolfBundle::new(position);

        assert_eq!(wolf.metadata.protocol_id(), 143);
        assert_eq!(wolf.metadata.resource_name(), "wolf");
        assert!(wolf.metadata.is_mob());
        assert!(wolf.spawn.is_persistent());
    }

    #[test]
    fn test_wolf_bundle_with_rotation() {
        let position = Position::new(10.0, 70.0, 20.0);
        let rotation = Rotation {
            yaw: 90.0,
            pitch: 0.0,
        };
        let wolf = WolfBundle::with_rotation(position, rotation);
        assert_eq!(wolf.rotation.yaw, 90.0);
    }
}
