...
impl RabbitBundle {
    pub fn new(position: Position) -> Self {
        let metadata = EntityMetadata::from_vanilla(&VanillaEntityType::RABBIT);
...
mod tests {
    ...
    #[test]
    fn test_rabbit_bundle_creation() {
        let position = Position::new(0.0, 64.0, 0.0);
        let rabbit = RabbitBundle::new(position);

        assert_eq!(rabbit.metadata.protocol_id(), 103);
        assert_eq!(rabbit.metadata.resource_name(), "rabbit");
        assert!(rabbit.metadata.is_mob());
        assert!(rabbit.spawn.is_persistent());
    }

    #[test]
    fn test_rabbit_bundle_with_rotation() {
        let position = Position::new(10.0, 70.0, 20.0);
        let rotation = Rotation {
            yaw: 90.0,
            pitch: 0.0,
        };
        let rabbit = RabbitBundle::with_rotation(position, rotation);
        assert_eq!(rabbit.rotation.yaw, 90.0);
    }
}
