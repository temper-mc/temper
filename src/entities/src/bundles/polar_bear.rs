...
impl PolarBearBundle {
    pub fn new(position: Position) -> Self {
        let metadata = EntityMetadata::from_vanilla(&VanillaEntityType::POLAR_BEAR);
...
mod tests {
    ...
    #[test]
    fn test_polar_bear_bundle_creation() {
        let position = Position::new(0.0, 64.0, 0.0);
        let polar_bear = PolarBearBundle::new(position);

        assert_eq!(polar_bear.metadata.protocol_id(), 99);
        assert_eq!(polar_bear.metadata.resource_name(), "polar_bear");
        assert!(polar_bear.metadata.is_mob());
        assert!(polar_bear.spawn.is_persistent());
    }

    #[test]
    fn test_polar_bear_bundle_with_rotation() {
        let position = Position::new(10.0, 70.0, 20.0);
        let rotation = Rotation {
            yaw: 90.0,
            pitch: 0.0,
        };
        let polar_bear = PolarBearBundle::with_rotation(position, rotation);
        assert_eq!(polar_bear.rotation.yaw, 90.0);
    }
}
