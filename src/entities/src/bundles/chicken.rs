...
/// Complete bundle to spawn a chicken in Bevy ECS.
///
/// This bundle contains all the necessary components to represent a chicken
...
impl ChickenBundle {
    pub fn new(position: Position) -> Self {
        let metadata = EntityMetadata::from_vanilla(&VanillaEntityType::CHICKEN);
...
mod tests {
    ...
    #[test]
    fn test_chicken_bundle_creation() {
        const EPSILON_F32: f32 = 1e-6;
        const EPSILON_F64: f64 = 1e-6;

        let position = Position::new(0.0, 64.0, 0.0);
        let chicken = ChickenBundle::new(position);

        // Verify vanilla metadata
        assert_eq!(chicken.metadata.protocol_id(), 25);
        assert_eq!(chicken.metadata.resource_name(), "chicken");
        assert!(chicken.metadata.is_mob());

        // Verify physical properties (using epsilon for floating point comparison)
        // Chicken dimensions: [width=0.4, height=0.7]
        assert!((chicken.physical.bounding_box.height() - 0.7).abs() < EPSILON_F64);
        assert!((chicken.physical.bounding_box.width() - 0.4).abs() < EPSILON_F64);
        assert!((chicken.physical.eye_height - 0.644).abs() < EPSILON_F32);
        assert!(!chicken.physical.fire_immune);

        // Verify combat properties
        assert!(chicken.combat.attackable);
        assert_eq!(chicken.combat.invulnerability_ticks, 0);

        // Verify spawn properties
        assert!(chicken.spawn.saveable);
        assert_eq!(chicken.spawn.limit_per_chunk, 4);
        assert!(chicken.spawn.is_friendly());
        assert!(chicken.spawn.is_persistent());
    }

    #[test]
    fn test_chicken_bundle_with_rotation() {
        let position = Position::new(10.0, 70.0, 20.0);
        let rotation = Rotation {
            yaw: 90.0,
            pitch: 0.0,
        };
        let chicken = ChickenBundle::with_rotation(position, rotation);

        assert_eq!(chicken.rotation.yaw, 90.0);
        assert_eq!(chicken.rotation.pitch, 0.0);
    }
}
