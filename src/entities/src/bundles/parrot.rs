...
impl ParrotBundle {
    pub fn new(position: Position) -> Self {
        let metadata = EntityMetadata::from_vanilla(&VanillaEntityType::PARROT);
...
mod tests {
    ...
    #[test]
    fn test_parrot_bundle_creation() {
        let position = Position::new(0.0, 64.0, 0.0);
        let parrot = ParrotBundle::new(position);

        assert_eq!(parrot.metadata.protocol_id(), 93);
        assert_eq!(parrot.metadata.resource_name(), "parrot");
        assert!(parrot.metadata.is_mob());
        assert!(parrot.spawn.is_persistent());
    }

    #[test]
    fn test_parrot_bundle_with_rotation() {
        let position = Position::new(10.0, 70.0, 20.0);
        let rotation = Rotation {
            yaw: 90.0,
            pitch: 0.0,
        };
        let parrot = ParrotBundle::with_rotation(position, rotation);
        assert_eq!(parrot.rotation.yaw, 90.0);
    }
}
