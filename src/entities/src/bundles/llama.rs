...
impl LlamaBundle {
    pub fn new(position: Position) -> Self {
        let metadata = EntityMetadata::from_vanilla(&VanillaEntityType::LLAMA);
...
mod tests {
    ...
    #[test]
    fn test_llama_bundle_creation() {
        let position = Position::new(0.0, 64.0, 0.0);
        let llama = LlamaBundle::new(position);

        assert_eq!(llama.metadata.protocol_id(), 76);
        assert_eq!(llama.metadata.resource_name(), "llama");
        assert!(llama.metadata.is_mob());
        assert!(llama.spawn.is_persistent());
    }

    #[test]
    fn test_llama_bundle_with_rotation() {
        let position = Position::new(10.0, 70.0, 20.0);
        let rotation = Rotation {
            yaw: 90.0,
            pitch: 0.0,
        };
        let llama = LlamaBundle::with_rotation(position, rotation);
        assert_eq!(llama.rotation.yaw, 90.0);
    }
}
