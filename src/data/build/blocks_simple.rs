...
"#;
    ...
    fs::write(blocks_dir.join("types.rs"), types_content).expect("Failed to write types.rs");
...
"#,
    ...
    fs::write(
        blocks_dir.join("block_entity_types.rs"),
...
"#,
    ...
    impl_content.push_str(
        "    const BLOCK_FROM_NAME_MAP: phf::Map<&'static str, &'static Block> = phf::phf_map! {\n",
    );
    for block in &blocks_assets.blocks {
        let const_name = const_block_name_from_block_name(&block.name);
...
"#,
    ...
    fs::write(blocks_dir.join("mod.rs"), mod_content).expect("Failed to write mod.rs");
