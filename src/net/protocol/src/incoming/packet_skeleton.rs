...
impl PacketSkeleton {
    ...
    pub async fn new<R: AsyncRead + Unpin>(
        ...
        match pak {
            ...
            Err(e) => {
                if !matches!(e, PacketError::DroppedConnection) {
    ...
    async fn read_uncompressed<R: AsyncRead + Unpin>(
        ...
        loop {
            ...
            if length < 1 {
                return Err(PacketError::MalformedPacket(Some(length as u8)));
            }

            // Sanity check to avoid maliciously large frames
            if length > 2097151 {
                let id = VarInt::read_async(reader).await?.0 as u8;
                return Err(PacketError::MalformedPacket(Some(id)));
            }
    ...
    async fn read_compressed<R: AsyncRead + Unpin>(
        ...
        loop {
            ...
            if packet_length < 1 {
                return Err(PacketError::MalformedPacket(Some(packet_length as u8)));
            }
            ...
            if data_length < 0 {
                return Err(PacketError::MalformedPacket(Some(data_length as u8)));
            }

            // Sanity checks to avoid huge memory allocations
            if packet_length > 2097151 || data_length > 8388608 {
                return Err(PacketError::MalformedPacket(Some(
            ...
            let (decompressed_data, checksum) =
                decompress(&compressed_buf, Format::Zlib).map_err(|err| {
                    ...
                    PacketError::CompressionError(GenericDecompressionError(msg))
                })?;
            ...
            if get_global_config().verify_decompressed_packets {
                ...
                if actual_checksum != expected {
                    ...
                    return Err(PacketError::CompressionError(ChecksumMismatch {
                        expected,
            ...
            if decompressed_data.len() != data_length as usize {
                ...
                error!(error_msg);
                return Err(PacketError::CompressionError(GenericDecompressionError(
                    error_msg,
                )));
            }
